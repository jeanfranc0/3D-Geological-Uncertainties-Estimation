...
class MEDA:
    def __init__(self, inputsXPor3D=(1, 1, 1, 1), inputsXRtp3D=(1, 1, 1, 1), inputsXNtg3D=(1, 1, 1, 1), inputsXPermi3D=(1, 1, 1, 1), inputXfluidUncertainties=(1, 1), n_outputs= (1), architecture='', pre_trained_model = None):
        ...
        self.model = self.__build_model(inputsXPor3D,
                                        ...
                                        n_outputs)
    def residual_block(self, input_tensor, filters, kernel_size):
    ...
    def encoderBlock(self, uncertainty, name):
        x = Conv3D(64, (3, 3, 3), padding='same')(uncertainty)
        x = BatchNormalization()(x)
        x = ReLU()(x)
        x = MaxPooling3D()(x)
        for _ in range(3):
            x = self.residual_block(x, 64, (3, 3, 3))
            x = MaxPooling3D()(x)
    ...
    def __build_model(self, inputsXPor3D, inputsXRtp3D, inputsXNtg3D, inputsXPermi3D, inputXfluidUncertainties, n_outputs):
        ...
        # Encode Porosity
        e1 = self.encoderBlock(Por3D, 'Por')
        
        # Encode Rock type
        e2 = self.encoderBlock(Rtp3D, 'Rtp')
        
        # Encode NTG
        e3 = self.encoderBlock(Ntg3D, 'Ntg')
        
        # Encode Permiability I
        e4 = self.encoderBlock(Permi3D, 'Permi')
        ...
        reg = Reshape((-1, 1))(reg)
        x = Bidirectional(LSTM(64, return_sequences=True))(reg)
        x = Bidirectional(LSTM(64, return_sequences=True))(x)
        reg = TimeDistributed(Dense(n_outputs))(x)
        reg = GlobalAveragePooling1D()(reg)
